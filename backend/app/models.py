...
class ChatRequest(BaseModel):
    ...
    max_tokens: Optional[int] = Field(
        2048, ge=1, le=8192, description="Maximum tokens to generate"
    )
    ...
    def validate_message_length(cls, v: str) -> str:
        ...
        if len(v) > MAX_QUERY_LENGTH:
            raise ValueError(
                f"Message too long. Maximum {MAX_QUERY_LENGTH} characters, got {len(v)}."
            )
        return v
...
class OutputValidation(BaseModel):
    ...
    source_citation_count: int = Field(
        0, description="Number of source citations found"
    )
    unsupported_claims_count: int = Field(
        0, description="Number of unsupported claims detected"
    )
    hallucination_markers_found: List[str] = Field(
        default_factory=list, description="Detected hallucination markers"
    )
    validation_time_ms: float = Field(
        0.0, description="Time taken for validation in ms"
    )
...
class RetrievalMetrics(BaseModel):
    ...
    initial_candidates: int = Field(
        ..., description="Number of initial vector search results"
    )
    after_reranking: Optional[int] = Field(
        None, description="Number of results after reranking"
    )
    ...
    hybrid_search_used: bool = Field(
        False, description="Whether hybrid search (BM25 + vector) was used"
    )
    hyde_used: bool = Field(
        False, description="Whether HyDE query expansion was applied"
    )
    hyde_time_ms: Optional[float] = Field(
        None, description="Time for HyDE generation in ms"
    )
    web_search_used: bool = Field(
        False, description="Whether web search fallback was triggered"
    )
    web_search_reason: Optional[str] = Field(
        None, description="Reason web search was triggered"
    )
    web_search_results: Optional[int] = Field(
        None, description="Number of web search results"
    )
    web_search_time_ms: Optional[float] = Field(
        None, description="Time for web search in ms"
    )
    avg_similarity_score: Optional[float] = Field(
        None, description="Average similarity score"
    )
    avg_rerank_score: Optional[float] = Field(None, description="Average rerank score")
    retrieval_time_ms: Optional[float] = Field(
        None, description="Time for vector retrieval in ms"
    )
    rerank_time_ms: Optional[float] = Field(
        None, description="Time for reranking in ms"
    )
    embedding_cache_hit: Optional[bool] = Field(
        None, description="Whether query embedding was served from cache"
    )
...
class ChatResponse(BaseModel):
    ...
    retrieval_metrics: Optional[RetrievalMetrics] = Field(
        None, description="Retrieval performance metrics"
    )
    output_validation: Optional[OutputValidation] = Field(
        None, description="Output validation and hallucination detection results"
    )
...
class RedisPoolStats(BaseModel):
    ...
    current_connections: Optional[int] = Field(
        None, description="Connections currently in use"
    )
    available_connections: Optional[int] = Field(
        None, description="Connections available in pool"
    )
...
class PostgresPoolStats(BaseModel):
    ...
    pool_timeout: Optional[float] = Field(
        None, description="Pool connection timeout in seconds"
    )
...
class CircuitBreakerStatus(BaseModel):
    ...
    total_calls: int = Field(
        0, description="Total number of calls through this breaker"
    )
    ...
    success_rate: Optional[float] = Field(
        None, description="Success rate as decimal (0-1)"
    )
    time_until_retry: Optional[float] = Field(
        None, description="Seconds until circuit may close (when open)"
    )
...
class CircuitBreakersStatus(BaseModel):
    ...
    healthy: bool = Field(
        ..., description="True if all breakers are closed or half-open"
    )
    ollama: CircuitBreakerStatus = Field(..., description="Ollama LLM circuit breaker")
    qdrant: CircuitBreakerStatus = Field(
        ..., description="Qdrant vector DB circuit breaker"
    )
    tavily: CircuitBreakerStatus = Field(
        ..., description="Tavily web search circuit breaker"
    )
...
class DeviceInfo(BaseModel):
    ...
    embedding_device: str = Field(
        ..., description="Actual device used for embeddings (cuda/mps/cpu)"
    )
    reranker_device: Optional[str] = Field(
        None, description="Actual device used for reranker (cuda/mps/cpu)"
    )
    cuda_available: bool = Field(False, description="Whether NVIDIA CUDA is available")
    cuda_device_name: Optional[str] = Field(
        None, description="NVIDIA GPU name if available"
    )
    mps_available: bool = Field(
        False, description="Whether Apple Silicon MPS is available"
    )
...
class HealthResponse(BaseModel):
    ...
    postgres_connected: bool = Field(
        False, description="Whether PostgreSQL is connected"
    )
    ...
    device_info: Optional[DeviceInfo] = Field(
        None, description="ML device configuration and GPU availability"
    )
    redis_pool: Optional[RedisPoolStats] = Field(
        None, description="Redis connection pool statistics"
    )
    postgres_pool: Optional[PostgresPoolStats] = Field(
        None, description="PostgreSQL connection pool statistics"
    )
    circuit_breakers: Optional[CircuitBreakersStatus] = Field(
        None, description="Circuit breaker status for resilience"
    )
    components: Optional[Dict[str, ComponentStatus]] = Field(
        None, description="Detailed component status"
    )
...
class ServiceStatus(BaseModel):
    ...
    status: str = Field(
        ..., description="Service status: connected, disconnected, degraded"
    )
    models_available: Optional[int] = Field(
        None, description="Number of models available (for Ollama)"
    )
    points_count: Optional[int] = Field(
        None, description="Number of indexed vectors (for Qdrant)"
    )
    pool_utilization: Optional[float] = Field(
        None, description="Connection pool utilization percentage"
    )
...
class HealthResponseSafe(BaseModel):
    ...
    reranker: Optional[Dict[str, Any]] = Field(
        None, description="Reranker status (enabled/loaded)"
    )
    device_info: Optional[DeviceInfo] = Field(
        None, description="ML device info (no sensitive data)"
    )
    circuit_breakers: Optional[Dict[str, str]] = Field(
        None, description="Circuit breaker states (state only)"
    )
...
class FeedbackRequest(BaseModel):
    ...
    message_index: Optional[int] = Field(
        None, description="Index of the response in session (0-based)"
    )
    helpful: bool = Field(
        ...,
        description="Whether the response was helpful (True=thumbs up, False=thumbs down)",
    )
    query_hash: Optional[str] = Field(
        None, description="Query hash for correlation with retrieval metrics"
    )
...
class QueryLogEntry(BaseModel):
    ...
    response_length: Optional[int] = Field(
        None, description="Response length in characters"
    )
    context_used: bool = Field(True, description="Whether RAG context was used")
    sources_count: Optional[int] = Field(
        None, description="Number of sources retrieved"
    )
    sources_returned: Optional[List[str]] = Field(
        None, description="Source document paths"
    )
    retrieval_scores: Optional[Dict[str, Any]] = Field(
        None, description="Retrieval score metrics"
    )
    hybrid_search_used: bool = Field(
        False, description="Whether hybrid search was used"
    )
...
class QueryAnalyticsSummary(BaseModel):
    ...
    model_distribution: Dict[str, int] = Field(
        default_factory=dict, description="Query count by model"
    )
    feature_usage: Dict[str, int] = Field(
        default_factory=dict, description="Feature usage counts"
    )
    queries_per_day: Dict[str, int] = Field(
        default_factory=dict, description="Query count by date"
    )
...
class QueryLogsFilter(BaseModel):
    ...
    hybrid_search_used: Optional[bool] = Field(
...
class ExperimentVariant(BaseModel):
    ...
    weight: float = Field(
        1.0, ge=0.0, le=1.0, description="Traffic weight for this variant (0.0-1.0)"
    )
    config: Dict[str, Any] = Field(
        default_factory=dict,
        description="Variant-specific configuration (model, temperature, etc.)",
    )
...
class ExperimentCreate(BaseModel):
    ...
    description: Optional[str] = Field(
        None, description="Experiment description and hypothesis"
    )
    ...
    start_at: Optional[str] = Field(
        None, description="Scheduled start time (ISO format)"
    )
...
class ExperimentUpdate(BaseModel):
    ...
    name: Optional[str] = Field(
        None, min_length=1, max_length=200, description="Updated experiment name"
    )
    ...
    variants: Optional[List[ExperimentVariant]] = Field(
        None, description="Updated variants (cannot change while running)"
    )
    traffic_percentage: Optional[float] = Field(
        None, ge=0.0, le=100.0, description="Updated traffic percentage"
    )
...
class ExperimentResponse(BaseModel):
    ...
    status: str = Field(
        ..., description="Current status: draft, running, paused, completed, archived"
    )
    variants: List[ExperimentVariant] = Field(..., description="Experiment variants")
    traffic_percentage: float = Field(
        ..., description="Traffic percentage in experiment"
    )
...
class VariantStats(BaseModel):
    ...
    conversion_rate: Optional[float] = Field(
        None, description="Conversion rate (conversions/sample_size)"
    )
    ...
    p50_latency_ms: Optional[float] = Field(None, description="50th percentile latency")
    p95_latency_ms: Optional[float] = Field(None, description="95th percentile latency")
    avg_metric_value: Optional[float] = Field(
        None, description="Average primary metric value"
    )
    std_metric_value: Optional[float] = Field(
        None, description="Standard deviation of metric"
    )
...
class StatisticalSignificance(BaseModel):
    ...
    relative_difference: float = Field(
        ..., description="Relative difference ((treatment-control)/control)"
    )
    ...
    is_significant: bool = Field(
        False, description="Whether result is statistically significant (p < 0.05)"
    )
    test_type: str = Field("t-test", description="Statistical test used")
    sample_size_adequate: bool = Field(
        False, description="Whether sample size is adequate for reliable results"
    )
    minimum_detectable_effect: Optional[float] = Field(
        None, description="MDE with current sample size"
    )
...
class ExperimentStatsResponse(BaseModel):
    ...
    duration_hours: Optional[float] = Field(
        None, description="Experiment duration in hours"
    )
    total_observations: int = Field(
        ..., description="Total observations across all variants"
    )
    ...
    winning_variant: Optional[str] = Field(
        None, description="Variant with best performance (if significant)"
    )
    recommendation: Optional[str] = Field(
        None, description="Action recommendation based on results"
    )
    computed_at: str = Field(
        ..., description="When statistics were computed (ISO format)"
    )
...
class VariantAssignmentResponse(BaseModel):
    ...
    variant_config: Dict[str, Any] = Field(
        ..., description="Configuration to apply for this variant"
    )
...
class ExperimentResultRecord(BaseModel):
    ...
    metric_name: str = Field(
        "latency_ms", description="Name of the metric being recorded"
    )
...
class APIKeyCreate(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=100, description="API key name/description"
    )
    ...
    expires_at: Optional[str] = Field(
        None, description="Optional expiration timestamp (ISO format)"
    )
...
class APIKeyResponse(BaseModel):
    ...
    last_used_at: Optional[str] = Field(
        None, description="Last usage timestamp (ISO format)"
    )
    key: Optional[str] = Field(
        None, description="API key value (only returned on creation)"
    )
    key_prefix: Optional[str] = Field(
        None, description="First 8 characters of API key for identification"
    )
...
class ReadinessCheck(BaseModel):
    ...
    ready: bool = Field(
        ..., description="Whether this component is ready to serve traffic"
    )
...
class ReadinessResponse(BaseModel):
    ...
    ready: bool = Field(
        ..., description="Overall readiness - true only if all critical checks pass"
    )
...
class TemplateVariable(BaseModel):
    ...
    name: str = Field(
        ..., description="Variable name used in the template (e.g., 'pod_name')"
    )
    type: str = Field(
        "string", description="Variable type: string, number, boolean, select"
    )
    description: Optional[str] = Field(
        None, description="Human-readable description of what this variable represents"
    )
    required: bool = Field(False, description="Whether this variable must be provided")
    default: Optional[Any] = Field(None, description="Default value if not provided")
    options: Optional[List[str]] = Field(
        None, description="Allowed values for 'select' type variables"
    )
    placeholder: Optional[str] = Field(
        None, description="Placeholder text to show in UI"
    )
...
class PromptTemplate(BaseModel):
    ...
    category: str = Field(
        ..., description="Template category (e.g., 'Kubernetes', 'Docker')"
    )
...
class RenderTemplateResponse(BaseModel):
    ...
    original_prompt: str = Field(
        ..., description="Original template prompt with placeholders"
    )
...
class LatencyMetrics(BaseModel):
    ...
    median_ms: Optional[float] = Field(
        None, description="Median (p50) latency in milliseconds"
    )
    p95_ms: Optional[float] = Field(
        None, description="95th percentile latency in milliseconds"
    )
    p99_ms: Optional[float] = Field(
        None, description="99th percentile latency in milliseconds"
    )
...
class RequestMetrics(BaseModel):
    ...
    total_requests_in_window: int = Field(
...
class TopQueryEntry(BaseModel):
    ...
    last_seen: str = Field(
        ..., description="Last time this query was seen (ISO format)"
    )
...
class RealtimeAnalyticsResponse(BaseModel):
    ...
    request_metrics: RequestMetrics = Field(
        ..., description="Request rate and error metrics"
    )
    ...
    retrieval_quality: RetrievalQualityMetrics = Field(
        ..., description="Retrieval similarity score metrics"
    )
    model_usage: Dict[str, ModelUsageStats] = Field(
        default_factory=dict, description="Model usage distribution"
    )
    top_queries: List[TopQueryEntry] = Field(
        default_factory=list, description="Top queries (anonymized, last hour)"
    )
...
class DriftCheckResponse(BaseModel):
    ...
    mean_shift_pct: Optional[float] = Field(
        None, description="Percentage shift in mean score from baseline"
    )
    std_shift_pct: Optional[float] = Field(
        None, description="Percentage shift in standard deviation"
    )
    median_shift_pct: Optional[float] = Field(
        None, description="Percentage shift in median score"
    )
    current: Optional[DriftMetricsResponse] = Field(
        None, description="Current window metrics"
    )
    baseline: Optional[DriftMetricsResponse] = Field(
...
class DriftStatusResponse(BaseModel):
    ...
    window_hours: int = Field(
        ..., description="Time window for collecting scores (hours)"
    )
    drift_threshold_pct: float = Field(
        ..., description="Threshold to trigger drift alert (percentage)"
    )
    warning_threshold_pct: float = Field(
        ..., description="Threshold to trigger warning (percentage)"
    )
    min_samples_required: int = Field(
        ..., description="Minimum samples needed for valid metrics"
    )
    ...
    redis_connected: bool = Field(
        ..., description="Whether Redis is available for storage"
    )
...
class DriftHistoryEntry(BaseModel):
    ...
    measurement_count: int = Field(
        ..., description="Number of metric measurements that day"
    )
...
class SetBaselineResponse(BaseModel):
    ...
    baseline: Optional[DriftMetricsResponse] = Field(
        None, description="The baseline metrics that were set"
    )
...
class SourceFreshnessModel(BaseModel):
    ...
    source_type: str = Field(
        ..., description="Documentation source type (e.g., kubernetes, terraform)"
    )
    last_download: Optional[str] = Field(
        None, description="ISO timestamp of last download"
    )
    last_commit_date: Optional[str] = Field(
        None, description="ISO timestamp of last git commit"
    )
    days_since_update: int = Field(
        ..., description="Days since last update (-1 if unknown)"
    )
    staleness_risk: str = Field(
        ..., description="Risk level: fresh, low, medium, high, or unknown"
    )
...
class FreshnessReportResponse(BaseModel):
    ...
    sources: List[SourceFreshnessModel] = Field(
        ..., description="Freshness status for all tracked sources"
    )
    total_sources: int = Field(..., description="Total number of tracked sources")
    stale_count: int = Field(
        ..., description="Number of sources with medium or higher staleness"
    )
