...
class ConversationContextConfig:
    ...
    def __post_init__(self):
        ...
        if self.history_messages_limit is None:
            self.history_messages_limit = getattr(
                settings, "conversation_context_history_limit", 3
            )
        if self.min_query_length_for_expansion is None:
            self.min_query_length_for_expansion = getattr(
                settings, "conversation_context_min_query_length", 5
            )
        if self.max_context_terms is None:
            self.max_context_terms = getattr(
                settings, "conversation_context_max_terms", 10
            )
...
class ConversationContextExpander:
    ...
    REFERENCE_PATTERNS = {
        ...
        # Reference phrases
        "the_same": re.compile(
            r"\b(the same|same thing|same one|the above)\b", re.IGNORECASE
        ),
        "mentioned": re.compile(
            r"\b(mentioned|described|discussed|explained)\b", re.IGNORECASE
        ),
        "previous": re.compile(
            r"\b(previous|earlier|before|last|prior)\b", re.IGNORECASE
        ),
    }
    ...
    DEVOPS_TERM_PATTERNS = [
        ...
        re.compile(
            r"\b(pod|pods|deployment|deployments|service|services|"
            ...
            r"horizontalpodautoscaler|hpa|verticalpodautoscaler|vpa)\b",
            re.IGNORECASE,
        ),
        # Container technologies
        re.compile(
            r"\b(docker|dockerfile|container|image|registry|"
            r"podman|containerd|cri-o|buildah|kaniko|"
            r"layer|volume|mount|compose|swarm)\b",
            re.IGNORECASE,
        ),
        # IaC and configuration
        re.compile(
            r"\b(terraform|ansible|helm|kustomize|"
            r"chart|playbook|role|task|module|provider|"
            r"state|plan|apply|destroy|workspace|"
            r"variable|output|resource|data|locals)\b",
            re.IGNORECASE,
        ),
        # Cloud providers
        re.compile(
            r"\b(aws|amazon|ec2|s3|rds|lambda|iam|vpc|"
            r"eks|ecs|fargate|cloudformation|"
            r"gcp|google cloud|gke|compute engine|cloud run|"
            r"azure|aks|arm|blob|"
            r"digitalocean|linode|vultr)\b",
            re.IGNORECASE,
        ),
        # CI/CD
        re.compile(
            r"\b(jenkins|gitlab|github actions|circleci|travis|"
            r"argocd|flux|spinnaker|tekton|"
            r"pipeline|workflow|stage|step|job|"
            r"build|deploy|release|rollout|rollback)\b",
            re.IGNORECASE,
        ),
        # Monitoring and observability
        re.compile(
            r"\b(prometheus|grafana|datadog|newrelic|"
            r"elasticsearch|kibana|logstash|elk|"
            r"jaeger|zipkin|opentelemetry|"
            r"metric|metrics|log|logs|trace|traces|"
            r"alert|alerting|dashboard|slo|sli|sla)\b",
            re.IGNORECASE,
        ),
        # Networking
        re.compile(
            r"\b(loadbalancer|load balancer|nginx|haproxy|envoy|"
            r"istio|linkerd|service mesh|"
            r"dns|ssl|tls|certificate|cert|"
            r"firewall|security group|network policy|"
            r"port|endpoint|url|domain)\b",
            re.IGNORECASE,
        ),
        # Common operations and concepts
        re.compile(
            r"\b(scale|scaling|autoscale|autoscaling|"
            ...
            r"debug|troubleshoot|error|issue|problem)\b",
            re.IGNORECASE,
        ),
    ]
    ...
    def _extract_context_from_history(
        ...
        recent_messages = conversation_history[
            -self.config.history_messages_limit * 2 :
        ]
    ...
    def _resolve_references(
        self, query: str, primary_topic: str, context_terms: Set[str]
    ...
    def expand_query(
        ...
        """
        result = ConversationContextResult(original_query=query, expanded_query=query)
